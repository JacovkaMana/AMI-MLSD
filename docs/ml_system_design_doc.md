...
**Схема в файле docs\architecture_diagram.png**

- **Описание компонентов**:
  - **Главный агент (AMI)**: Координирует работу системы, принимает запросы пользователя и распределяет задачи между специализированными агентами
  - **Специализированные агенты**: Обрабатывают задачи в своих доменах (мир, персонажи и т.д.)
  - **Инструменты**: Предоставляют справочные данные для генерации контента
  - **Языковая модель (Mistral)**: Генерирует контент на основе промптов от агентов
  - **Хранилище данных**: Временное хранение в оперативной памяти (Redis/Memcached в будущих версиях)

#### 4.2. Описание инфраструктуры и масштабируемости 

- **Текущая инфраструктура**: 
  - Локальные разработческие среды (ноутбуки разработчиков)
  - Консольное приложение на Python 3.10+
  - Виртуальные окружения через Poetry
  - Кэширование в оперативной памяти

- **Плюсы**: 
  - Нулевые эксплуатационные затраты на инфраструктуру
  - Быстрое прототипирование и итерации
  - Простота отладки

- **Минусы**: 
  - Ограниченная производительность для параллельных запросов
  - Отсутствие отказоустойчивости
  - Невозможность горизонтального масштабирования

- **Планы по масштабированию**:
  1. **Короткий срок**: Контейнеризация приложения (Docker)
  2. **Средний срок**: Развертывание на облачном сервисе (Yandex Cloud)
  3. **Долгосрочный**: 
     - Микросервисная архитектура
     - Горизонтальное масштабирование агентов
     - Балансировка нагрузки между экземплярами
     - Кластеризация Redis для кэширования

#### 4.3. Требования к работе системы  

- **Надежность**:
  - Доступность: 95% в рабочие часы (10:00-20:00)
  - Допустимое время простоя: не более 3 часов в месяц
  - Частота сбоев: не более 1 на 50 запросов

- **Производительность**:
  - Время отклика для типичных запросов: ≤ 3 минут
...
Результаты в Locust_2025-06-21-21h32.html
